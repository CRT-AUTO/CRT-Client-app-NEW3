...
import { LayoutDashboard, MessageSquare, Settings, LogOut, MessageCircle, AlertTriangle, Users, ShieldCheck } from 'lucide-react';
...
export default function Layout({ user, voiceflowInitialized = false }: LayoutProps) {
  ...
  // Use a ref to track if layout has fetched initial user data
  const initialDataFetchedRef = useRef<boolean>(false);
  const lastUserFetchRef = useRef<number>(0);
  
  // Initial user loading - only if user prop isn't provided
  useEffect(() => {
    // If user is provided from props, use that
    if (user) {
      setLocalUser(user);
      // Get admin status
      isAdmin().then(adminStatus => setUserIsAdmin(adminStatus));
      return;
    }
    ...
    const fetchUserData = async () => {
      // Rate limit fetches - don't fetch again if we did it < 5 seconds ago
      const now = Date.now();
      if (now - lastUserFetchRef.current < 5000) return;
